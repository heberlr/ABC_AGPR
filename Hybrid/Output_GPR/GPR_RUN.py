...
import sys
sys.path.append('../../../src')
from ABC import ABC_SMC,ABC_MCMC # Change to run with parallel
from AGPR import AdapGP

def Model_AGPR(Par):
  ParLocal = np.reshape(Par, (1, Par.size))
  with open('modelGPR.pkl', 'rb') as f:
    gpr = pickle.load(f)
...
if __name__ == '__main__':
  ...
  #outSMC = ABC_SMC(Model_AGPR, data, DW, UP,'CalibSMC_GPR.dat',epsilon, NumAccept=1000)
  # Calling the method MCMC-AGPR
  outMCMC = ABC_MCMC(Model_AGPR, data, DW, UP,'CalibMCMC_GPR.dat',epsilon[-1], NumAccept=1000,var_trasition=0.1)
